...
void ImageManager::FrameAniRender(string strKey, HDC hdc, int destX, int destY)
{
	Image * img = FindImage(strKey);
	if (img)
	{

		int frameMaxX = img->GetMaxFrameX();
		int frameMaxY = img->GetMaxFrameY();

		int x = (img->GetFrameX() + 1) % frameMaxX;
		int y = (img->GetFrameY() + 1) % frameMaxY;
	
		img->SetFrameX(x);
		img->SetFrameY(y);

		img->FrameRender(hdc, destX, destY, x, y);
